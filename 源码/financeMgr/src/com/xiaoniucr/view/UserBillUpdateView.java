 ...
 *
 */
public class UserBillUpdateView extends JFrame {
	 ...
	 * Create the frame.
	 */
	public UserBillUpdateView(final Integer id,final UserBillManageView frame) {
		...
		try {
			...
			Card c = cardDao.getById(bill.getCardId());
			titleText.setText(bill.getTitle());
			payDateText.setText(bill.getPayDate());
			cardComboBox.setSelectedItem(c);
			billTypeComboBox.setSelectedItem(bill.getBillType() == 0 ? "收入":"支出");
			amountText.setText(bill.getAmount()+"");
			remarkTextPane.setText(bill.getRemark());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		saveBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(bill.getBillType() == 0){
					balance = c.getBalance() - bill.getAmount();
				}else{
					balance = c.getBalance() + bill.getAmount();
				}
				//金额更新
				...
				boolean flag = billDao.update(bill);
				if (flag) {
